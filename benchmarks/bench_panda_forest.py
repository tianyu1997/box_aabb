#!/usr/bin/env python
"""
benchmarks/bench_panda_forest.py - Panda 7-DOF Box Forest 拓展测试

在随机 3D 障碍物场景下，测试 Panda 机器人关节空间的 box forest 拓展。
记录详细的各环节耗时、运行次数、每个 box 的拓展日志。

用法：
    python -m benchmarks.bench_panda_forest
    python -m benchmarks.bench_panda_forest --n-obs 10 --max-boxes 200
    python -m benchmarks.bench_panda_forest --max-boxes 200 --max-seeds 3000 --max-depth 30
"""
...
def random_scene_3d(
    ...
    Panda 实际最大臂展约 0.855m，工作空间以机器人基座为中心。
    障碍物在 xy 方向均匀分布在 0.3~0.9 倍半径的球冠区域，
    z 方向分布在桌面以上。
    """
    scene = Scene()
...
def grow_forest_panda(
    ...
    Args:
        early_stop_min_size: 归一化尺寸 vol^(1/d) 的早停阈值
        use_cache: 是否使用全局缓存（False = 冷启动，不删除已有缓存）
    """
    ...
    recent_sizes: List[float] = []  # vol^(1/d) 归一化尺寸
    step = 0
    ...
    def _try_add_box(seed_q, source=""):
        ...
        # 详细日志输出到文件（每 10 个 box INFO，其余 DEBUG）
    ...
    # save cache（冷启动时合并到已有缓存，热启动直接覆盖）
...
def generate_report(result: dict, args) -> str:
    lines = []
    lines.append("=" * 90)
    lines.append("  Panda 7-DOF Box Forest 拓展详细报告")
    lines.append("=" * 90)
    lines.append(f"  时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"  机器人: Panda 7-DOF")
...
def main():
    parser = argparse.ArgumentParser(
        description="Panda 7-DOF Box Forest 拓展详细测试"
    )
    ...
    output_dir = Path("benchmarks/output") / f"panda_forest_{timestamp}"
    ...
    # 所有 logger: INFO 级别写文件，不传播到控制台
    for log_name in ["bench_panda", "planner", "box_aabb"]:
    ...
    console_logger.warning("  Panda 7-DOF Box Forest 拓展测试")
    console_logger.warning(
    ...
    # 详细日志到文件
    logger.info("=" * 70)
    logger.info("  Panda 7-DOF Box Forest 拓展测试")
