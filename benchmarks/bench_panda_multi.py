#!/usr/bin/env python
"""
benchmarks/bench_panda_multi.py - Panda 7-DOF 多场景对比测试

测试不同障碍物数量 × 冷/热缓存 的 box forest 拓展表现。
每个障碍物数量运行两次：第一次清除缓存(冷)，第二次使用缓存(热)。

用法：
    python -m benchmarks.bench_panda_multi
    python -m benchmarks.bench_panda_multi --obs-list 5 10 15 20 --max-boxes 200
"""
...
# 复用 bench_panda_forest 的核心函数
from benchmarks.bench_panda_forest import random_scene_3d, grow_forest_panda
...
def run_single(
    ...
    result = grow_forest_panda(
        ...
        rng_seed=rng_seed + 1000,  # 拓展种子与场景种子分开
...
def generate_comparison_report(all_results: list, args) -> str:
    ...
    lines = []
    lines.append("=" * 100)
    lines.append("  Panda 7-DOF Box Forest 多场景对比测试报告")
    lines.append("=" * 100)
...
def main():
    parser = argparse.ArgumentParser(
        description="Panda 7-DOF 多场景对比测试"
    )
    ...
    output_dir = Path("benchmarks/output") / f"panda_multi_{timestamp}"
    ...
    for log_name in ["bench_multi", "bench_panda", "planner", "box_aabb"]:
    ...
    console_logger.warning("  Panda 7-DOF 多场景对比测试")
    ...
    # 详细日志到文件
    logger.info("=" * 80)
    logger.info("  Panda 7-DOF 多场景对比测试")
