// SafeBoxForest — Connectivity implementation
...
BridgeResult bridge_islands(
    ...
    // Pre-compute one representative center per island (centroid of all box centers)
    // and a small set of "frontier" boxes (closest to overall centroid of other islands)
    // to avoid O(N^2) all-pairs comparison.
    int ndim = 0;
    ...
    for (int attempt = 0; attempt < max_attempts && n_islands > 1; ++attempt) {
        // Find closest pair of islands by centroid distance (O(I^2))
        ...
        // For the two closest islands, find closest box pair
        // Use frontier sampling: only check K closest boxes to opposing centroid
