// SafeBoxForest — PathSmoother implementation
...
PathSmoother::shortcut(const std::vector<Eigen::VectorXd>& path,
    ...
    for (int iter = 0; iter < max_iters; ++iter) {
        ...
        // Try to connect i and j directly
        if (!checker_->check_segment(current[i], current[j], segment_resolution_)) {
            // Shortcut successful — remove intermediate waypoints
...
PathSmoother::box_aware_shortcut(const std::vector<Eigen::VectorXd>& path,
    ...
    for (int iter = 0; iter < max_iters; ++iter) {
        ...
        // Check if both endpoints are in the same box or adjacent boxes
        ...
        if (box_i && box_j) {
            if (box_i->id == box_j->id) {
                // Same box → shortcut is trivially safe
                safe_shortcut = true;
            } else {
                // Check if adjacent
...
PathSmoother::smooth_moving_average(const std::vector<Eigen::VectorXd>& path,
    ...
    for (int iter = 0; iter < iterations; ++iter) {
        ...
        for (int i = 1; i < n - 1; ++i) {
            ...
            // Box-aware: clamp to nearest containing box
