...
// ═════════════════════════════════════════════════════════════════════════════
//  FFBCallbacks — hook points for LECT/GridStore during FFB descent
// ═════════════════════════════════════════════════════════════════════════════
//
//  refine_check:  Called at step 4c when AABB says "collision".
//    If the grid says "no collision", returns false → node accepted as free.
//    Signature: (node_idx, obs_compact, n_obs) → true if collision.
//
//  on_split:  Called at step 4f after two children are created and FK computed.
//    LECT uses this to derive grid for the new leaf nodes and store frames.
//    Signature: (left_idx, right_idx, parent_idx, left_fk, right_fk)
//
//  propagate_grid:  Called alongside AABB propagate_up.
//    LECT uses this to union children grids → parent grid.
//    Signature: (parent_idx, left_idx, right_idx)
//
struct FFBCallbacks {
    // Refine collision check: returns true = collision, false = safe.
    // Called when AABB says collision; grid can override to "safe".
    std::function<bool(int node_idx, const float* obs_compact, int n_obs)>
        refine_check;

    // Called after lazy split creates two children (with FK available).
    std::function<void(int left_idx, int right_idx, int parent_idx,
                       const FKState& left_fk, const FKState& right_fk)>
        on_split;

    // Called during propagate_up for grid union.
    std::function<void(int parent_idx, int left_idx, int right_idx)>
        propagate_grid;
};
...
public:
    ...
    FFBResult find_free_box(const Eigen::VectorXd& seed,
                            ...
                            int safety_check_depth = 0,
                            const FFBCallbacks* callbacks = nullptr) const;
...
private:
    ...
    // If callbacks given, also propagates grid union.
    void propagate_up(int node_idx, const FFBCallbacks* callbacks = nullptr) const;
