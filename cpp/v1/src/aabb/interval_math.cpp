// SafeBoxForest — Interval arithmetic implementation
// Ported from v4/_interval_fk_core.pyx
...
// ─── I_sin: conservative interval enclosure for sin([lo, hi]) ───────────────
Interval I_sin(double lo, double hi) {
    ...
    // Normalize lo into [0, 2π)
    double lo_n = std::fmod(lo, TWO_PI);
    if (lo_n < 0.0) lo_n += TWO_PI;
    double hi_n = lo_n + w;

    // Check if π/2 (max of sin) is in the interval
    // Use ceil/floor to check: any integer k such that lo_n <= π/2 + 2kπ <= hi_n
    {
        double k_lo = std::ceil((lo_n - HALF_PI) / TWO_PI);
        double check = HALF_PI + k_lo * TWO_PI;
        if (check <= hi_n + 1e-15) r_hi = 1.0;
    }

    // Check if 3π/2 (min of sin) is in the interval
    {
        double k_lo = std::ceil((lo_n - THREE_HALF_PI) / TWO_PI);
        double check = THREE_HALF_PI + k_lo * TWO_PI;
...
// ─── I_cos: conservative interval enclosure for cos([lo, hi]) ───────────────
Interval I_cos(double lo, double hi) {
    ...
    // Check if 0 (max of cos) is in the interval
    {
        double k_lo = std::ceil(lo_n / TWO_PI);
        double check = k_lo * TWO_PI;
        if (check <= hi_n + 1e-15) r_hi = 1.0;
    }

    // Check if π (min of cos) is in the interval
    {
        double k_lo = std::ceil((lo_n - PI) / TWO_PI);
        double check = PI + k_lo * TWO_PI;
        if (check <= hi_n + 1e-15) r_lo = -1.0;
    }
...
// ─── Build DH joint interval matrix ─────────────────────────────────────────
// Modified DH matrix:
// [ct, -st, 0, a]
// [st*ca, ct*ca, -sa, -d*sa]
// [st*sa, ct*sa,  ca,  d*ca]
...
void build_dh_joint(double alpha, double a,
    ...
    // Helper for interval multiplication of two intervals
    auto imul = [](double a_lo, double a_hi, double b_lo, double b_hi,
                   double& r_lo, double& r_hi) {
        double p1 = a_lo * b_lo, p2 = a_lo * b_hi;
        double p3 = a_hi * b_lo, p4 = a_hi * b_hi;
        r_lo = std::min({p1, p2, p3, p4});
        r_hi = std::max({p1, p2, p3, p4});
    };
    (void)imul;

    // Helper: interval × scalar
    ...
    // Row 3: [0, 0, 0, 1] hardcoded
    for (int i = 0; i < 16; ++i) A_lo[i] = A_hi[i] = 0.0;
    A_lo[15] = A_hi[15] = 1.0;

    // Row 0: [ct, -st, 0, a]
    A_lo[0] = ct_lo; A_hi[0] = ct_hi;
    A_lo[1] = -st_hi; A_hi[1] = -st_lo;  // -st: negate flips interval
    // A_lo[2] = A_hi[2] = 0.0;  // already zero
    A_lo[3] = A_hi[3] = a;
...
void imat_mul_dh(const double T_lo[16], const double T_hi[16],
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 4; ++j) {
            ...
            // k = 0, 1, 2 (skip k=3 since A[3][0..2] = 0)
            for (int k = 0; k < 3; ++k) {
                int t_idx = i * 4 + k;
                int a_idx = k * 4 + j;
            ...
            // else A[3][j] = 0, contribution is 0
