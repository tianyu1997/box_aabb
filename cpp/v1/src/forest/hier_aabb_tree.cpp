...
FFBResult HierAABBTree::find_free_box(const Eigen::VectorXd& seed,
                                       ...
                                       int max_depth, double min_edge,
                                       int safety_check_depth,
                                       const FFBCallbacks* callbacks) const {
    FFBResult result;
    ...
    while (true) {
        ...
        //
        // Three-level collision check:
        //   Level 1: AABB per-link SAT (fastest, most conservative)
        //   Level 2: Grid bitfield probe (zero-inflation, via refine_check callback)
        //   Level 3: Recursive child-decomposition (safety_check_depth > 0)
        if (store_.get_u8(node, node_layout::OFF_HAS_AABB)) {
            bool collide;
            if (safety_check_depth > 0) {
                collide = promotion_collide_check(idx, obs_compact, n_obs, safety_check_depth);
            } else {
                collide = aabbs_collide_obs(store_.aabb_ptr(node), n_aabb_slots,
                                            obs_compact, n_obs);
            }

            // Grid refinement: if AABB says collision but grid says safe, override
            if (collide && callbacks && callbacks->refine_check) {
                collide = callbacks->refine_check(idx, obs_compact, n_obs);
            }
        ...
        if (left_idx == -1) {
            ...
            // FKState saved for on_split callback
            FKState saved_left_fk, saved_right_fk;
            ...
            // Left child FK: incremental from parent
            {
                ...
                std::memcpy(&saved_left_fk, &left_fk, sizeof(FKState));
                if (going_left) {
            ...
            // Right child FK: incremental from SAVED parent (not from left!)
            {
                ...
                std::memcpy(&saved_right_fk, &right_fk, sizeof(FKState));
                if (!going_left) {
            ...
            // Callback: on_split — derive grid / store frames for new children
            if (callbacks && callbacks->on_split) {
                callbacks->on_split(li, ri, idx, saved_left_fk, saved_right_fk);
            }
    ...
    if (result.success()) {
        ...
        if (parent_idx >= 0) {
            propagate_up(parent_idx, callbacks);
        }
    }
...
void HierAABBTree::propagate_up(int idx, const FFBCallbacks* callbacks) const {
    ...
    while (idx >= 0) {
        ...
        if (store_.has_aabb(idx)) {
            ...
            if (std::abs(cur[0] - old_lo0) < 1e-12f &&
                ...
                // Still propagate grid even if AABB didn't change
                if (callbacks && callbacks->propagate_grid)
                    callbacks->propagate_grid(idx, left_idx, right_idx);
                break;
            }
        } else {
        ...
        // Propagate grid union alongside AABB
        if (callbacks && callbacks->propagate_grid)
            callbacks->propagate_grid(idx, left_idx, right_idx);
...
// Enhanced with early-out: check parent AABB first as quick accept.
// If parent AABB is collision-free, no need to recurse (since parent
// AABB is a conservative bound for the entire subtree).
// If parent AABB collides and remaining_depth > 0, recurse into children's
// tighter AABBs — this avoids false positives due to union inflation.
bool HierAABBTree::promotion_collide_check(int idx, const float* obs_compact,
    ...
    // Quick accept: if this node's AABB doesn't collide, subtree is safe
    if (store_.has_aabb(idx) &&
        !aabbs_collide_obs(store_.aabb(idx), n_slots, obs_compact, n_obs))
        return false;

    // Try to refine: recurse into children's tighter AABBs
    if (remaining_depth > 0) {
        int left_idx = store_.left(idx);
        if (left_idx >= 0) {
            int right_idx = store_.right(idx);
            if (store_.has_aabb(left_idx) && store_.has_aabb(right_idx)) {
                // Both children must be collision-free
                if (promotion_collide_check(left_idx, obs_compact, n_obs, remaining_depth - 1))
                    return true;
                return promotion_collide_check(right_idx, obs_compact, n_obs, remaining_depth - 1);
            }
        }
    }

    // Cannot refine: return conservative collision
    // (parent AABB collided or is absent, and we can't recurse further)
    return true;
}
