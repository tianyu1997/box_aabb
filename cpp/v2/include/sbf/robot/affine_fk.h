// SafeBoxForest v2 — Affine-Arithmetic Forward Kinematics
// Module: sbf::robot
//
...
//
// Usage for GCPC pruning:
//   AAFKResult res = aa_compute_fk(robot, intervals);
//   double lo[3], hi[3];
//   aa_position_bounds(res, frame_idx, lo, hi);
//   // compare with out_aabb to decide pruning
//
#pragma once
...
//   R[r][c] = Σ_{k=0..2} A[r][k]·B[k][c] + (c==3 ? A[r][3] : 0)
// because B[3][c] = δ_{c,3}.
inline AAMatrix4 aa_mat_mul(const AAMatrix4& A, const AAMatrix4& B) {
    AAMatrix4 R;
    for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 4; ++c) {
            // sum = A[r][0]*B[0][c] + A[r][1]*B[1][c] + A[r][2]*B[2][c]
...
// ─── AA FK Result ───────────────────────────────────────────────────────────
//   prefix[i] = T₀ × T₁ × … × T_{i-1}   (cumulative transform)
//   prefix[0] = Identity
//   prefix[n+1] = full chain (possibly with tool)
struct AAFKResult {
    // Heap-allocated to avoid 34 × 12 × 80B ≈ 32 KB stack blow-up
    std::vector<AAMatrix4> prefix;  // prefix[0..n_tf-1]
    int n_tf = 0;
    bool valid = false;
};
...
// lo[d], hi[d] for d ∈ {0=x, 1=y, 2=z}.
inline void aa_position_bounds(const AAMatrix4& T,
                               double lo[3], double hi[3]) {
    // Position is column 3 of rows 0–2 → m[3], m[7], m[11]
