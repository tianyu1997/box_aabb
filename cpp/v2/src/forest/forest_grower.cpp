// SafeBoxForest v2 — ForestGrower implementation
...
ForestGrower::ForestGrower(const Robot& robot,
    ...
    : robot_(&robot), checker_(&checker), tree_(&tree),
      ...
      root_sampler_(robot.joint_limits(), config.seed),
      boundary_sampler_(robot.joint_limits(), config.boundary_expand_epsilon),
    ...
    while (consec < max_miss) {
        ...
        if (!expand_queue.empty()) {
            ...
            auto seeds = boundary_sampler_.generate_boundary_seeds(
                ...
                goal_pt, n_samp, root_sampler_.rng(), effective_eps);
