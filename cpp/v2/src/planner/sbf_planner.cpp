// SafeBoxForest v2 — SBFPlanner implementation (v4 logic: bidirectional tree
    ...
    double timeout)
{
    ...
    if (n_random_boxes > 0 && elapsed() < timeout) {
        ...
        int max_bfs_depth = 5;
        double rand_bfs_eps = config_.ffb_min_edge_relaxed * 0.5;
    ...
    if (any_disconnected) {
        ...
        for (int pi = 0; pi < n_pairs; ++pi) {
            ...
            if (!s_box) s_box = forest_.find_containing(s);
    ...
    Eigen::VectorXd& proxy_out, bool& used_proxy)
{
    ...
    auto create_box_at = [&](const Eigen::VectorXd& seed,
                             double min_edge) -> int {
        if (tree_.is_occupied(seed)) return -2;
    ...
    // Step C: Fallback — aggressive FFB
    std::cout << "    [anchor] " << origin_tag
...
bool SBFPlanner::grow_boxes(const Eigen::VectorXd& start,
    ...
    if (!connected && !skip_final_adjacency) {
        ...
        for (int gr = 0; gr < gap_rounds; ++gr) {
            ...
            if (src_island == tgt_island) break;
