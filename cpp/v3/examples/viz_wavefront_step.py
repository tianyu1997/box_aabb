"""
examples/viz_wavefront_step.py — 可视化 *一次* 波前扩展 (Wavefront BFS Step)
...
用法:
    cd safeboxforest/v3
    python examples/viz_wavefront_step.py [--seed 42] [--pre-grow 20]
"""
...
# ── 复用主模块中的核心类 ──────────────────────────────────────────────
# 直接从同目录的 viz_2d_forest_grower 导入
...
from viz_2d_forest_grower import (
    ...
    FFBEngine, BoxInfo, ForestGrower2D,
...
def compose_gif(frames_dir: Path, gif_path: Path, duration_ms: int = 800):
    """PNG 帧 → GIF."""
    ...
    # step0: 1.5s, step1: 2s, child steps: 1.2s each, final: 3s
    durations = []
...
def main():
    ...
    out_dir = Path(__file__).resolve().parent.parent / "results" / f"viz_wavefront_step_{ts}"
    ...
    # ── 选择 parent box ──────────────────────────────────────────────
    # 策略: 试探性地从 BFS 队列中挑出能实际扩展出 child 的 box.
    # 逐个尝试, 第一个能成功生成 >=1 child 的就选它.
    ...
    try:
        ok = compose_gif(frames_dir, gif_path)
        if ok:
