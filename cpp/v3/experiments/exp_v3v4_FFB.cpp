// ═══════════════════════════════════════════════════════════════════════════
//  Experiment: v3 vs v4 find_free_box (FFB) latency comparison  — v3 side
//
...
//    pipeline     = fast() = IFK + SubAABB(n_sub=1)
//    split_order  = ROUND_ROBIN (v3 only mode)
//
//  CSV (stdout):
...
//
//  Run:
//    build\Release\exp_v3v4_FFB.exe > v3_ffb_results.csv 2>v3_ffb_log.txt
// ═══════════════════════════════════════════════════════════════════════════
...
// ─── Robot factories (identical DH params as v4 side) ────────────────────────
...
// ─── Random obstacle generator (deterministic, identical to v4 side) ────────
...
int main(int argc, char* argv[]) {
        ...
        for (int ci = 0; ci < n_counts; ++ci) {
            ...
            for (int si = 0; si < n_scenes; ++si) {
                ...
                for (int qi = 0; qi < n_queries; ++qi) {
                    ...
                    std::printf("v3,%s,%d,%d,%d,%.1f,%d,%d,%d,%d\n",
                                re.name, n_obs, si, qi,
                                time_us,
                                result.success() ? 1 : 0,
                                result.n_new_nodes,
                    ...
                    if ((qi + 1) % 50 == 0) {
                        std::fprintf(stderr,
                            "[v3] %s n_obs=%d scene=%d/%d query=%d/%d  "
                            "tree=%d nodes\n",
                ...
                ++scene_done;
                std::fprintf(stderr,
                    "[v3] Scene %d/%d done (%s, %d obs, seed=%d, "
                    "final tree=%d nodes)\n",
    ...
    std::fprintf(stderr, "[v3] All %d scenes completed.\n", total_scenes);
    return 0;
}
