// SafeBoxForest v2 — Core type definitions (shared across all modules)
...
struct BoxNode {
    ...
    int parent_box_id    = -1;  // box this was expanded from (-1 = root)
    int expand_face_dim  = -1;  // dimension of the face used for expansion
    int expand_face_side = -1;  // 0 = lo face, 1 = hi face
    int root_id          = -1;  // which root tree this box belongs to
