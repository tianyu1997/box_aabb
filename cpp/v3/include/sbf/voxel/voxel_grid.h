...
//  voxel_grid.h
//
//  VoxelGrid wraps a FlatBrickMap (open-addressing hash map).
//  Provides:
//    • fill_aabb()    — mark voxels overlapping a Cartesian AABB
//    • fill_hull16()  — turbo scanline rasterisation of Conv(B₁∪B₂)⊕Ball
//    • merge()  / collides()  — bitwise OR / AND over brick maps
//    • count_occupied() / occupied_volume() — popcount-based queries
// ═══════════════════════════════════════════════════════════════════════════
#pragma once
...
//
//  Replaces std::unordered_map for ~2-3× faster lookup/insert.
//  Linear probing with power-of-2 table size, 70% max load factor.
//  API: operator[], find()/end(), begin()/end(), size(), clear().
// ═══════════════════════════════════════════════════════════════════════════
class FlatBrickMap {
    struct Slot {
        BrickCoord key;
...
public:
    ...
    struct const_iterator {
        ...
        // For collides()/count_colliding() dereference pattern: it->second
        const BitBrick& second() const { return ptr->value; }
    };

    // Find: returns const_iterator
    ...
    iterator begin() {
        return {table_.data(), table_.data() + capacity_};
    }
    iterator end() {
        return {table_.data() + capacity_, table_.data() + capacity_};
    }
    ...
    /// Pre-allocate the hash table to hold at least `n_hint` entries
    /// without triggering grow().  No-op if capacity is already sufficient.
    void reserve(int n_hint) {
        int target = next_pow2(n_hint * 10 / 7 + 1);  // 70% load factor headroom
        ...
        (void)old_size;  // suppress warning
    }
...
public:
    ...
    /// @param safety_pad  extra radius padding for fill_hull16.
    ///        Default = √3·Δ/2 (ensures voxel-cube corners are covered).
    ///        Set to 0 for pure-geometry envelope comparison.
    ...
    /// Pre-allocate internal brick storage for a bounding volume.
    /// Estimates the number of bricks from axis-aligned bounds and reserves
    /// the hash map to avoid repeated re-hashing during fill operations.
    /// @param lo  world-space lower corner [x, y, z]
    /// @param hi  world-space upper corner [x, y, z]
    ...
    /// Rasterise Conv(B₁ ∪ B₂) ⊕ Ball(r_link + √3·Δ/2) via turbo scanline.
    ///
    ...
    void fill_hull16(const float* prox_iv,
                     const float* dist_iv,
                     double link_radius)
    {
        ...
        // ── Fast-path: degenerate hull (slopes ≈ 0) ───────────────────
        //  When prox ≈ dist, hull ≈ B₁ ⊕ Ball(r_eff).  AABB fill is exact.
        ...
        //
        // Process the YZ loop in 8×8 brick tiles.  For each (by,bz) tile:
        //   1. Brick-level early-out: test tile centre with inflated radius;
        //      if the entire tile is outside the hull, skip 64 scanlines.
        //   2. Accumulate X-direction bitmasks into stack-local BitBricks.
        //   3. Flush to the global brick map once per tile — reduces hash
        //      probes from O(cells × x_bricks) to O(yz_bricks × x_bricks).
        //
        ...
        // Brick-level early-out: inflate r_eff by half-diagonal of the
        // 8×8 cell tile in YZ so that a single centre test is conservative.
        // Max cell-centre distance from tile centre: sqrt(3.5²+3.5²)·δ.
        ...
        // Stack-local BitBrick accumulator per X-brick (reused per tile).
        static constexpr int MAX_XB = 128;
        BitBrick local_bricks[MAX_XB];

        if (n_xb > MAX_XB) {
            // Fallback: original per-cell path for extremely wide hulls
            for (int cz = cz0; cz <= cz1; ++cz) {
            ...
            for (int by = by0; by <= by1; ++by) {
                ...
                // ── Brick-level early-out ────────────────────────────
                {
                    ...
                    auto [t0, t1] = find_t_range_yz(
                        wy_c, wz_c, r_test_sq,
                        b1_lo, b1_hi, slo, shi);
                    if (t0 > t1) continue;   // entire tile missed
                }
                ...
                // ── Scanline cells in this (by,bz) tile ─────────────
                for (int cz = cz_s; cz <= cz_e; ++cz) {
                    ...
                    for (int cy = cy_s; cy <= cy_e; ++cy) {
                        ...
                        // Fill X range into local bricks
                        int cx = cxs;
                        while (cx <= cxe) {
                            int bx    = brick_of(cx);
                            int lx0   = local_of(cx);
    ...
    /// Keep only voxels present in *both* grids (bitwise AND).
    /// Bricks absent in `other` are zeroed; bricks absent in `this` stay absent.
    void intersect_inplace(const VoxelGrid& other) {
        for (auto e : bricks_) {
            auto it = other.bricks_.find(e.key);
            if (it != other.bricks_.end()) {
                bricks_[e.key] = e.value & it.second();
            } else {
                // Zero out — brick absent in other, no overlap
                bricks_[e.key] = BitBrick{};
            }
        }
    }
    ...
    /// Insert a brick directly (for deserialization / bulk construction).
...
private:
    ...
    /// Exploits the BitBrick layout to batch-OR entire rows within a brick.
