...
from .load_data import (
    SnapshotData, SceneData, Obstacle, AABB,
    load_snapshot_data,
)
from .robot_viz import robot_traces
from .envelope_viz import aabb_traces, envelope_traces
...
def _parse_snapshot_envelope(ed: Dict[str, Any]):
    """Extract AABB lists from the envelope section of a snapshot."""
    link_aabbs = [
        AABB(link=a["link"], lo=np.array(a["lo"]), hi=np.array(a["hi"]))
        for a in ed.get("link_aabbs", [])
    ]
    sub_aabbs = [
        AABB(link=a["link"], lo=np.array(a["lo"]), hi=np.array(a["hi"]),
             seg=a.get("seg", -1))
        for a in ed.get("sub_aabbs", [])
    ]
    return link_aabbs, sub_aabbs
...
def plot_snapshot_3d(data,
    ...
    # ── Envelope AABBs ──
    if data.envelope:
        link_aabbs, sub_aabbs = _parse_snapshot_envelope(data.envelope)
        if show_full_aabb:
            for t in envelope_traces(link_aabbs, prefix="AABB "):
                fig.add_trace(t)
        if show_sub_aabb:
            for t in envelope_traces(sub_aabbs, prefix="Sub "):
                fig.add_trace(t)
