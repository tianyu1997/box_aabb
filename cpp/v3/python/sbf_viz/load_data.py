"""JSON loading utilities and lightweight data classes for sbf_viz."""
...
def load_envelope_data(path: str) -> EnvelopeData:
    ...
    for nd in d.get("nodes", []):
        la = [AABB(link=a["link"], lo=np.array(a["lo"]), hi=np.array(a["hi"]))
              for a in nd.get("link_aabbs", [])]
        sa = [AABB(link=a["link"], lo=np.array(a["lo"]), hi=np.array(a["hi"]),
                   seg=a.get("seg", -1))
              for a in nd.get("sub_aabbs", [])]
