// SafeBoxForest v2 — Interval arithmetic implementation
...
Interval I_sin(double lo, double hi) {
    ...
    // Outward-round endpoints to cover std::sin's ≤ 1 ULP error
    double r_lo = std::min(outward_lo(s_lo), outward_lo(s_hi));
    double r_hi = std::max(outward_hi(s_lo), outward_hi(s_hi));
...
Interval I_cos(double lo, double hi) {
    ...
    // Outward-round endpoints to cover std::cos's ≤ 1 ULP error
    double r_lo = std::min(outward_lo(c_lo), outward_lo(c_hi));
    double r_hi = std::max(outward_hi(c_lo), outward_hi(c_hi));
...
void build_dh_joint(double alpha, double a,
    ...
    // Outward-round every entry: lo toward −∞, hi toward +∞.
    // Covers ≤ 1 ULP rounding error per product.
    for (int i = 0; i < 16; ++i) {
        A_lo[i] = outward_lo(A_lo[i]);
        A_hi[i] = outward_hi(A_hi[i]);
    }
}
...
void imat_mul_dh(const double T_lo[16], const double T_hi[16],
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 4; ++j) {
            ...
            // Outward-round to cover accumulated mul+add rounding
            R_lo[i * 4 + j] = outward_lo(lo_sum);
            R_hi[i * 4 + j] = outward_hi(hi_sum);
        }
    }
}
