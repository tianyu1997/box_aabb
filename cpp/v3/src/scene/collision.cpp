// SafeBoxForest v2 �?Collision detection implementation (SAT)
...
AabbCollisionChecker::AabbCollisionChecker(const Robot& robot,
...
void AabbCollisionChecker::pack_obstacles() {
...
int AabbCollisionChecker::n_aabb_slots() const {
    return robot_->n_active_links() + robot_->n_ee_aabb_slots();
}

bool AabbCollisionChecker::check_config(const Eigen::VectorXd& q) const {
    ++n_checks_;
...
bool AabbCollisionChecker::check_box(const std::vector<Interval>& intervals) const {
    ++n_checks_;
    ...
    extract_link_aabbs_hybrid(state, intervals, *robot_, aabb.data(),
                              aa_crossover_width_);
    if (n_ee_slots > 0) {
...
bool AabbCollisionChecker::check_segment(const Eigen::VectorXd& q1,
