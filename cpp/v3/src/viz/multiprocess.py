"""
src/viz/multiprocess.py — 多进程 ForestGrower 2D 可视化
...
from .forest_grower_2d import build_random_scene
from .render import ROOT_COLORS, plot_snapshot, compose_gif
...
def main():
    ...
    out_dir = v3_root / "results" / f"viz_mp_{ts}"
    ...
    sp_elapsed = None
    if args.compare:
        from .forest_grower_2d import ForestGrower2D
        print("[mp-viz] 单进程对比 ...")
        t0 = time.time()
    ...
    md = [
        ...
        f"- workers: {len(grower.subtrees)}, max_boxes: {cfg.max_boxes}",
        f"- boxes: {mp_n_boxes}, edges: {mp_n_edges}",
        f"- elapsed: {mp_elapsed:.2f}s",
        "",
        f"![forest](forest_grower_mp.gif)",
    ]
    (out_dir / "README.md").write_text("\n".join(md) + "\n", encoding="utf-8")

    print(f"\n{'='*60}")
    print(f"输出目录: {out_dir}")
    print(f"  boxes: {mp_n_boxes}, edges: {mp_n_edges}")
