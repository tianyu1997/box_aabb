// ═══════════════════════════════════════════════════════════════════════════
//  Experiment: v3 vs v4 ForestGrower benchmark  — v4 side
//
...
//    n_obstacles  = {5, 10, 20}
//    modes        = {Wavefront, RRT}
//    scene_seeds  = 0..4   (5 random scenes per config)
//    max_boxes    = 300
//    timeout      = 30 s
//    pipeline     = fast() = IFK + LinkIAABB(n_sub=1)
//
//  CSV (stdout):
//    version,robot,n_obstacles,mode,scene_seed,
//    grow_time_ms,n_boxes,n_ffb_success,n_ffb_fail,
//    n_components,start_goal_connected,total_volume,tree_size,
//    phase_root_ms,phase_expand_ms,phase_adj_ms
//
//  Build:
//    cmake --build build --config Release --target exp_v3v4_grower
//
//  Run:
//    build\Release\exp_v3v4_grower.exe > v4_grower_results.csv 2>v4_grower_log.txt
// ═══════════════════════════════════════════════════════════════════════════
...
// ─── Robot factories (identical to exp_v3v4_FFB) ─────────────────────────────
...
// ─── Random obstacle generator (identical to exp_v3v4_FFB) ───────────────────
...
int main(int argc, char* argv[]) {
    ...
    std::fprintf(stderr, "[v4] All %d runs completed.\n", total);
    return 0;
}
