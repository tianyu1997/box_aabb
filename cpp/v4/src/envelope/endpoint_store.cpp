// ═══════════════════════════════════════════════════════════════════════════
//  SafeBoxForest v4 — EndpointStore implementation
//  Includes binary persistence (.frames, FRM3/FRM4 format) with
//  dirty tracking and incremental append-only save.
//  迁移自 v3 frame_store.cpp (FrameStore → EndpointStore)
// ═══════════════════════════════════════════════════════════════════════════
...
EndpointStore::EndpointStore(const Robot& robot, int initial_capacity)
    : n_endpoints_(robot.n_joints() + (robot.has_tool() ? 1 : 0))
...
EndpointStore::EndpointStore(EndpointStore&& o) noexcept
    : n_endpoints_(o.n_endpoints_)
...
EndpointStore& EndpointStore::operator=(EndpointStore&& o) noexcept {
...
void EndpointStore::mark_dirty(int node_idx) {
    if (mmap_.is_open()) {
        ++n_dirty_;
        return;
    }
...
void EndpointStore::store_from_fk(int node_idx, const FKState& fk) {
    ensure_capacity(node_idx + 1);
    float* dst = get_endpoints_mut(node_idx);

    for (int k = 0; k < n_endpoints_; ++k) {
...
void EndpointStore::store_endpoints(int node_idx, const float* endpoints) {
    ensure_capacity(node_idx + 1);
    std::memcpy(get_endpoints_mut(node_idx), endpoints,
                n_endpoints_ * 6 * sizeof(float));
    set_valid(node_idx, true);
    mark_dirty(node_idx);
}

void EndpointStore::set_has_endpoints(int node_idx, bool val) {
    if (node_idx >= 0 && node_idx < capacity_)
        set_valid(node_idx, val);
}

// ─── Endpoint-level union / refine ──────────────────────────────────────────

void EndpointStore::union_endpoints(int dst_idx, int a_idx, int b_idx) {
    const float* a = get_endpoints(a_idx);
    const float* b = get_endpoints(b_idx);
    if (!a || !b) return;

    ensure_capacity(dst_idx + 1);
    float* dst = get_endpoints_mut(dst_idx);

    for (int f = 0; f < n_endpoints_; ++f) {
        int off = f * 6;
...
bool EndpointStore::refine_endpoints(int dst_idx, int union_src_idx) {
    float* dst = get_endpoints_mut(dst_idx);
    const float* src = get_endpoints(union_src_idx);
...
void EndpointStore::save(const std::string& path) {
    if (mmap_.is_open()) {
        flush_mmap();
        return;
    }
    ...
    if (!fp) throw std::runtime_error("EndpointStore::save: cannot open " + path);
    ...
    u[0] = MAGIC_V4;
    u[1] = VERSION_V4;
    u[2] = static_cast<uint32_t>(n_endpoints_);
    ...
    const size_t data_per_rec = static_cast<size_t>(n_endpoints_) * 6 * sizeof(float);
...
void EndpointStore::save_incremental() {
    if (save_path_.empty()) return;
    save_incremental(save_path_);
}

void EndpointStore::save_incremental(const std::string& path) {
    if (mmap_.is_open()) {
        flush_mmap();
        return;
    }
...
void EndpointStore::load(const std::string& path) {
    FILE* fp = std::fopen(path.c_str(), "rb");
    if (!fp) throw std::runtime_error("EndpointStore::load: cannot open " + path);

    uint8_t hdr[MMAP_HEADER_SIZE];
    if (std::fread(hdr, 1, MMAP_HEADER_SIZE, fp) !=
        static_cast<size_t>(MMAP_HEADER_SIZE)) {
        std::fclose(fp);
        throw std::runtime_error("EndpointStore::load: truncated header in " + path);
    }
    ...
    if (magic == MAGIC_V4 && ver == VERSION_V4) {
        n_endpoints_    = static_cast<int>(u[2]);
        ...
        throw std::runtime_error(
            "EndpointStore::load: unknown magic/version in " + path);
    }
}

int EndpointStore::n_valid() const {
    int count = 0;
    for (int i = 0; i < capacity_; ++i)
...
void EndpointStore::ensure_capacity(int n_nodes) {
    ...
    if (mmap_.is_open()) {
        ...
        data_.resize(static_cast<size_t>(new_cap) * n_endpoints_ * 6, 0.f);
...
void EndpointStore::write_mmap_header() {
...
void EndpointStore::read_mmap_header() {
    assert(mmap_.is_open());
    const char* p = mmap_.ptr;
    auto* u = reinterpret_cast<const uint32_t*>(p);
    if (u[0] != MAGIC_V4 || u[1] != VERSION_V4)
        throw std::runtime_error("EndpointStore::read_mmap_header: bad magic/version");
    n_endpoints_    = static_cast<int>(u[2]);
    n_active_links_ = static_cast<int>(u[3]);
...
void EndpointStore::grow_mmap(int new_cap) {
    assert(mmap_.is_open());
...
void EndpointStore::create_mmap(const std::string& path, int initial_capacity) {
...
void EndpointStore::load_mmap(const std::string& path) {
    if (mmap_.is_open()) close_mmap();

    mmap_ = mmap_util::open_rw(path, 0);
    if (mmap_.size < static_cast<size_t>(MMAP_HEADER_SIZE))
        throw std::runtime_error("EndpointStore::load_mmap: file too small " + path);
