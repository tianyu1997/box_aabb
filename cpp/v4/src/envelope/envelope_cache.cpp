// ═══════════════════════════════════════════════════════════════════════════
//  SafeBoxForest v4 — Unified Cache Module implementation
//  迁移自 v3 envelope_cache.cpp，适配 v4 命名
// ═══════════════════════════════════════════════════════════════════════════
...
uint8_t EnvelopeCache::available_layers(const std::string& dir)
{
    ...
    if (fs::exists(endpoints_path(dir))) layers |= CACHE_ENDPOINTS;
                                   ...
                                   CacheMetadata& meta)
{
    ...
    try {
        ...
        meta.source_method = static_cast<EndpointSource>(
        ...
        meta.n_endpoints = j.value("n_endpoints",
                            j.value("n_frames", 0));  // backward compat
