...
On a KUKA IIWA14 (7-DOF, 4~active links), the system builds 9{,}489
collision-free boxes in 24.7\,s (including 11.7\,s parallel bridging
with 15~threads), achieves 100\% success on 5~benchmark queries with
path lengths 2.050--8.920\,rad and query times 0.15--5.56\,s.
...
\begin{itemize}[nosep]
  ...
  \item End-to-end validation on KUKA IIWA14 7-DOF: 24.7\,s build /
    9{,}489~boxes / 100\% success rate (\cref{sec:experiments}).
\end{itemize}
...
\begin{enumerate}[nosep]
  ...
  \item Greedily merge by score; collision-verify via
    \texttt{check\_box} + LECT \texttt{intervals\_collide\_scene}.
...
\midrule
Total build time & 24.7\,s & LECT load + grow + coarsen + bridge \\
Bridge time & 11.7\,s & 15-thread parallel RRT \\
Box count (post-coarsen) & 9{,}489 & 5~seed trees merged \\
Island count (post-bridge) & 1 & Fully connected \\
\bottomrule
\end{tabular}
\end{table}
...
\emph{P2 analysis.}
15-thread parallelisation reduces bridging from 17.7\,s to 11.7\,s.
...
\begin{enumerate}[nosep]
  ...
  \item KUKA IIWA14 validation: 24.7\,s build / 9{,}489~boxes /
    100\% success / 2.050--8.920\,rad paths.
\end{enumerate}
...
\texttt{coarsen.score\_threshold} & 50.0 & Merge score threshold \\
