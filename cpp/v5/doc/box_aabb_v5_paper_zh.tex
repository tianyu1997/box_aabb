...
系统以 2.3 秒中位数完成安全盒构建（连通性驱动生长自动跳过桥接与二次粗化），
5 对典型查询 100\% 成功率，路径长度 1.908--5.116 rad，
查询时间 0.018--0.142 秒。
\end{abstract}
...
\begin{enumerate}[nosep]
  ...
  \item 按评分排序贪心执行；碰撞验证使用 \texttt{hull\_region\_safe}：
    (1)~成员盒覆盖剪枝（已被已知安全成员盒覆盖的子区域直接跳过），
    (2)~保守 AABB 检查，(3)~LECT 树遍历（缓存节点连杆包络 + 增量区间传递）
...
\begin{enumerate}[nosep]
  \item \textbf{ThreadPool}：$N_{\mathrm{hw}} - 1$ 线程（本实验 5 线程），并行执行多个 RRT-Connect
...
\midrule
总构建时间 & 2.7 s & 含 LECT 加载 + 生长 + 粗化 \\
生长时间 & 0.7 s & 5 线程协调，连通后 +4000 盒 \\
盒数（过滤后） & 5{,}010 & 5 棵种子树, 生长阶段全连通 \\
桥接/二次粗化 & 跳过 & 生长阶段已实现全连通 \\
\bottomrule
\end{tabular}
\end{table}
...
P4: 连通性驱动终止 & 构建速度 & 跳过桥接+二次粗化, 构建 $-$74\% & \checkmark 生效 \\
...
\textbf{P2 分析}：5 线程并行将桥接从 3.1s 降至 2.0s。
Cancel flag 在岛合并时取消剩余任务，避免无用计算。
所有查询结果与串行版本完全一致。

\textbf{P4 分析}：连通性驱动生长终止——去除固定超时，
改为所有种子树连通后额外生长 4{,}000 个盒（\texttt{post\_connect\_extra\_boxes}）即终止。
当生长阶段实现全连通时，桥接与二次粗化阶段直接跳过。
构建中位数从 9.2s 降至 2.3s（4$\times$ 加速），
所有 5 对查询保持 100\% 成功率与相同路径长度。
...
\begin{enumerate}[nosep]
  ...
  \item 在 KUKA IIWA14 7-DOF 上验证：2.3s 中位构建，5 对查询 100\% 成功率。
\end{enumerate}
...
\texttt{gcs.max\_gcs\_verts} & 200 & GCS corridor 粗化阈值 \\
