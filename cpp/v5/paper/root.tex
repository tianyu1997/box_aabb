...
A per-depth tightening heuristic selects the dimension whose bisection
minimises the worst-case child link-envelope volume:
...
completely bypassing the full endpoint-envelope reconstruction.
...
from the parent, and only the affected links are extracted from the FK state.
...
The LECT cache file records the endpoint source used during cache
generation. At runtime, the planner may switch to a different endpoint
source for newly created nodes, while previously cached nodes retain
their original data in a separate channel.
The dual-channel architecture separates a conservative channel for
collision certification from a tighter channel for sampled or analytical
sources, allowing collision queries to prefer the tighter channel when
available and fall back to the conservative one otherwise.
This enables loading a cache generated from broad coverage while using
the faster incremental path for runtime expansion.

\subsection{Endpoint Source Dispatcher}\label{sec:endpoint_dispatch}
The endpoint-source module provides a unified dispatch mechanism that
selects among four endpoint-generation modes: interval forward
kinematics, boundary sampling, analytical evaluation, and cached
critical-point lookup. The output is always a fixed-length endpoint
envelope array containing $n_\text{endpoints} \times 6$ floating-point
values.

\emph{Quality ordering (tightness).}
Sources are ranked by envelope tightness:
The tightness ordering is interval forward kinematics, boundary
sampling, analytical evaluation, and cached critical-point lookup.
The cached critical-point mode uses analytical evaluation at the
boundaries and cache reuse in the interior, producing strictly tighter
bounds than standalone analytical evaluation.
