...
\emph{惰性分裂。}
LECT 仅从根节点开始，在需要更细分辨率时按需分裂。
沿维度 $d$ 分裂时，前缀 $[\bT^{0:d-1}]$ 共享（43\% FK 节省）。
分裂维度由按深度懒惰评估的探针确定：对每个候选维度
...
\emph{运行时源覆盖。}
LECT 缓存文件记录了生成时使用的端点来源。
运行时，规划器可切换到不同的端点来源，新创建的节点采用当前来源，
而已缓存节点保留其原始数据在独立通道中。
双通道架构将保守通道与紧致通道分离，碰撞查询优先使用更紧密的通道，
缺失时回退到保守通道。
这使得可以加载广覆盖缓存，同时在运行时扩展中使用更快的增量路径。

\subsection{端点源分发器}\label{sec:endpoint_dispatch}
% 代码: include/sbf/envelope/endpoint_source.h

端点来源模块提供统一分发机制，
根据来源类别选择四种模式：区间 FK、边界采样、解析评估和缓存临界点检索。
输出始终为固定长度的端点包络数组，
包含 $n_\text{endpoints} \times 6$ 个浮点数。

\emph{质量排序（紧密性）。}
紧密性顺序为区间 FK、边界采样、解析评估和缓存临界点检索。
缓存临界点模式在边界采用解析评估，在内部复用缓存，
因此其界限严格紧于单独的解析评估。
