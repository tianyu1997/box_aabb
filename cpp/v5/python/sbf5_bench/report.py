...
def freeze_table(s5_data: dict) -> str:
    """Generate Table 5 LaTeX: freeze vs direct comparison.

    Expects s5_data = {"rows": [{"source", "width", "k_star", "ratio", "speedup"}, ...]}.
    """
    rows = s5_data.get("rows", [])
    if not rows:
        return "% No freeze data."

    lines = [
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{Freeze vs.\ direct comparison (IIWA14, 50 random boxes,",
        r"adaptive freeze depth $k^*{\in}\{0,4\}$).}",
        r"\label{tab:freeze}",
        r"\setlength{\tabcolsep}{3pt}",
        r"\small",
        r"\begin{tabular}{@{}lcccc@{}}",
        r"\toprule",
        r"Source & Width & $k^*$ & Ratio & Speedup \\",
        r"\midrule",
    ]
    ...
    for r in rows:
        ...
        lines.append(
            f"  {source} & {width} & {k_star} & {ratio:.2f} & {speedup}$\\times$ \\\\"
        )
