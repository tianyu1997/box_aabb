...
bool Z4GridCache::open(const std::string& path,
    ...
    if (exists) {
        ...
        if (fd_ < 0) {
            fprintf(stderr, "[Z4GridCache] open(%s) failed: %s\n",
        ...
        if (::pread(fd_, &hdr, sizeof(hdr), 0) != sizeof(hdr)) {
            ...
            fprintf(stderr, "[Z4GridCache] incompatible cache, recreating\n");
            ::close(fd_); fd_ = -1;
            exists = false;
        } else {
    ...
    if (!exists) {
        ...
        if (fd_ < 0) {
            fprintf(stderr, "[Z4GridCache] create(%s) failed: %s\n",
                    path.c_str(), strerror(errno));
            return false;
        }
...
void Z4GridCache::remap_file(size_t new_file_size, size_t new_mmap_size) {
    ...
    if (::ftruncate(fd_, static_cast<off_t>(new_file_size)) != 0) {
        fprintf(stderr, "[Z4GridCache] ftruncate(%zu) failed: %s\n",
    ...
    if (data_ == MAP_FAILED) {
        data_ = nullptr;
        fprintf(stderr, "[Z4GridCache] remap mmap failed\n");
        return;
    }
...
void Z4GridCache::grow_index() {
    ...
    if (::ftruncate(fd_, static_cast<off_t>(new_file_size)) != 0) {
        fprintf(stderr, "[Z4GridCache] grow_index ftruncate(%zu) failed: %s\n",
                new_file_size, strerror(errno));
        return;
    }
    file_size_ = new_file_size;
    ...
    if (old_data_used > 0) {
        ...
        while (remaining > 0) {
            ...
            if (rd != static_cast<ssize_t>(chunk)) {
                fprintf(stderr,
                    "[Z4GridCache] grow_index pread failed at offset %zu\n",
                    off_in_data);
                return;
            }
    ...
    if (data_ == MAP_FAILED) {
        data_ = nullptr;
        fprintf(stderr, "[Z4GridCache] grow_index mmap failed\n");
        return;
    }
    mmap_size_ = new_mmap_size;
    hdr = reinterpret_cast<Header*>(data_);
    ...
    fprintf(stderr,
        "[Z4GridCache] grow_index: %d → %d slots (%d entries, data=%zuMB chunked)\n",
        old_cap, new_cap, hdr->index_size, old_data_used / (1024*1024));
}
