                        ...
                        double timeout_ms)
{
    ...
    if (config_.use_v6_cache && !config_.lect_no_cache) {
        ...
        if (cache_mgr_->init(robot_.fingerprint(), robot_.name(),
            ...
            fprintf(stderr, "[PLN] V6 cache: EP safe=%d/%d unsafe=%d/%d, dir=%s\n",
                    cache_mgr_->ep_cache(0).size(),
                    cache_mgr_->ep_cache(0).capacity(),
                    cache_mgr_->ep_cache(1).size(),
                    cache_mgr_->ep_cache(1).capacity(),
                    cache_mgr_->cache_dir().c_str());
        } else {
            fprintf(stderr, "[PLN] V6 cache init failed, falling back to V5\n");
            cache_mgr_.reset();
        }
    }
...

    {
        ...
        fprintf(stderr, "[PLN] greedy1=%.0fms (%d->%d)\n", greedy1_ms, n_sweep1, n_greedy1);
        fflush(stderr);
    }
    ...
    // 3. Bridge S↔T on coarsened boxes (larger boxes → better RRT success)
    {
        ...
        int n_after_bridge = (int)boxes_.size();
        {
            ...
            fprintf(stderr, "[PLN] bridge=%.0fms added=%d boxes=%d islands=%d\n",
                    bridge_ms, bridge_count, n_after_bridge, (int)islands_post_bridge.size());
        }
        fflush(stderr);
        ...
        if (repaired > 0)
            fprintf(stderr, "[PLN] repaired %d non-geometric bridge edges\n", repaired);
        fflush(stderr);
    }
    ...
    int n_pre_coarsen2 = (int)boxes_.size();
    {
        ...
        fprintf(stderr, "[PLN] sweep2=%.0fms (%d->%d)\n", sweep2_ms, n_pre_coarsen2, n_sweep2);
        fflush(stderr);
        ...
        if (!build_deadline_reached()) {
            ...
            fprintf(stderr, "[PLN] greedy2=%.0fms (%d->%d)\n", greedy2_ms, n_sweep2, n_greedy2);
            fflush(stderr);
        }
    }
    ...
    fprintf(stderr, "[PLN] grow=%d coarsen1=%d bridge=%d coarsen2=%d filter=%d\n",
            (int)raw_boxes_.size(), n_sweep1, n_pre_coarsen2, (int)boxes_.size(), n3);
    fflush(stderr);
    ...
    adj_ = compute_adjacency(boxes_);
    {
        ...
        fprintf(stderr, "[PLN] final: boxes=%d islands=%d edges=%d\n",
                (int)boxes_.size(), (int)islands_final.size(), edges_final/2);
        fflush(stderr);
    }
...
int SBFPlanner::warmup_lect(int max_depth, int n_paths, int seed)
{
    ...
    if (config_.use_v6_cache && !config_.lect_no_cache) {
        ...
        if (cache_mgr_->init(robot_.fingerprint(), robot_.name(),
                             ...
                             config_.lect_cache_dir)) {
            lect_->set_cache_manager(cache_mgr_.get());
            fprintf(stderr, "[WRM] V6 cache: EP safe=%d/%d unsafe=%d/%d, dir=%s\n",
                    cache_mgr_->ep_cache(0).size(),
                    cache_mgr_->ep_cache(0).capacity(),
                    cache_mgr_->ep_cache(1).size(),
                    cache_mgr_->ep_cache(1).capacity(),
            ...
            fprintf(stderr, "[WRM] V6 cache init failed, falling back to V5\n");
            cache_mgr_.reset();
        }
    }
                                 ...
                                 const std::vector<Eigen::VectorXd>& seed_points)
{
    ...
    const int nd = robot_.n_joints();
    fprintf(stderr, "[PLN] multi-goal RRT: %d seed points\n", (int)seeds.size());
    for (int i = 0; i < static_cast<int>(seeds.size()); i++) {
        fprintf(stderr, "[PLN]   seed[%d] = (", i);
        for (int d = 0; d < nd; d++)
            fprintf(stderr, "%.3f%s", seeds[i][d], d < nd - 1 ? ", " : "");
        fprintf(stderr, ")\n");
    }
    fflush(stderr);
...

    {
        ...
        fprintf(stderr, "[PLN] relaxed_sweep1=%.0fms (%d->%d)\n",
                rsweep1_ms, n_sweep1, n_rsweep1);
        fflush(stderr);
    }

    {
        ...
        fprintf(stderr, "[PLN] articulation points: %d (adj+artic %.0fms)\n",
                (int)bridge_ids_1.size(), artic1_ms);
        fflush(stderr);
        ...
        double greedy1_ms = std::chrono::duration<double, std::milli>(
            std::chrono::steady_clock::now() - t_greedy1).count();
        fprintf(stderr, "[PLN] greedy1=%.0fms (%d->%d)\n", greedy1_ms, n_pre_greedy1, n_greedy1);
        fflush(stderr);
        ...
        // Cluster merge (3+box at a time)
        {
            ...
            fprintf(stderr, "[PLN] cluster1=%.0fms (%d->%d, %d clusters)\n",
                    cl1_ms, cl1.boxes_before, cl1.boxes_after,
                    cl1.clusters_formed);
            fflush(stderr);
        }
    }
    ...
    // 3. Bridge all islands (on coarsened boxes — larger boxes → better RRT success)
    {
            ...
            std::chrono::steady_clock::now() - t_adj_br).count();
        {
            ...
            fprintf(stderr, "[PLN] pre-bridge adj: %.0fms (boxes=%d, islands=%d)\n",
                    adj_br_ms, (int)boxes_.size(), (int)pre_islands.size());
        }
        fflush(stderr);
            ...
            std::chrono::steady_clock::now() - t_brg).count();
        {
            ...
            fprintf(stderr, "[PLN] bridge_all: +%d boxes (%.0fms, total=%d, islands=%d)\n",
                    bridged, brg_ms, (int)boxes_.size(), (int)brg_islands.size());
        }
        fflush(stderr);
    ...
    int n_greedy2 = n_pre_coarsen2;  // updated below
    {
...

        {
            ...
            fprintf(stderr, "[PLN] relaxed_sweep2=%.0fms (%d->%d)\n",
                    rsweep2_ms, n_sweep2, n_rsweep2);
            fflush(stderr);
        }
        ...
        // Cluster merge on coarsen2
        {
            ...
            fprintf(stderr, "[PLN] cluster2=%.0fms (%d->%d, %d clusters)\n",
                    cl2_ms, cl2.boxes_before, cl2.boxes_after,
                    cl2.clusters_formed);
            fflush(stderr);
        }
    }
    ...
    fprintf(stderr, "[PLN] grow=%d coarsen1=%d bridge=%d coarsen2=%d filter=%d (%.0fms)\n",
            (int)raw_boxes_.size(), n_sweep1, n_pre_coarsen2, n_greedy2, n3, filter_ms);
    fflush(stderr);
    ...
    if (!seed_points.empty()) {
        ...
        if (sp_bridged > 0) {
            fprintf(stderr, "[PLN] seed-point bridge: +%d boxes (%d seed_points)\n",
                    sp_bridged, (int)seed_points.size());
            fflush(stderr);
        }
    }

    {
        ...
        fprintf(stderr, "[PLN] island sizes:");
        for (int s : island_sizes) fprintf(stderr, " %d", s);
        fprintf(stderr, "\n");
        fflush(stderr);
    }
                              ...
                              const Obstacle* obs, int n_obs)
{
    ...
    fprintf(stderr, "[QRY] query: start_id=%d goal_id=%d n_boxes=%d\n",
    ...
    if (use_obs && lect_) {
        ...
        int created = bridge_s_t(
            ...
            std::chrono::steady_clock::time_point::max());
        if (created > 0) {
            fprintf(stderr, "[QRY] bridge: added %d boxes\n", created);
...

    {
        ...
        fprintf(stderr, "[QRY] islands=%d largest=%d start_in=%d goal_in=%d\n",
                (int)islands.size(), (int)largest_set.size(),
                !start_isolated, !goal_isolated);
                ...
                for (int ci = 0; ci < (int)candidates.size(); ++ci) {
                    ...
                    if (!rrt_path.empty()) {
                        ...
                        fprintf(stderr, "[QRY] proxy: %s box %d -> proxy %d (dist=%.3f, rrt=%dwp, try=%d, %.0fms)\n",
                                label, orig_id, c.id, std::sqrt(c.dist), (int)link_path.size(), ci+1, total_ms);
                        return c.id;
                    }
                }
    ...
    if (config_.use_gcs) {
        ...
        if (!dij.found) {
            fprintf(stderr, "[QRY] dijkstra FAIL: proxy_s=%d proxy_g=%d — trying RRT direct\n",
                    proxy_start_id, proxy_goal_id);
            // Direct RRT fallback
            ...
            if (!direct.empty()) {
                ...
                fprintf(stderr, "[QRY] RRT direct (dij-fail): OK (%d pts)\n", (int)path.size());
            } else {
                return result;
            }
        }
        ...
        if (path.empty() && dij.found) {
            ...
            if (path.empty()) {
                fprintf(stderr, "[QRY] extract_waypoints FAIL — trying RRT fallback\n");
                ...
                if (start_isolated && goal_isolated) {
                    ...
                    if (!mid.empty()) {
                        ...
                        start_link_path.clear();
                        goal_link_path.clear();
                        fprintf(stderr, "[QRY] RRT fallback (both proxy): OK (%d pts)\n", (int)path.size());
                    }
                    ...
                    if (!mid.empty()) {
                        ...
                        path.front() = start;
                        start_link_path.clear();
                        fprintf(stderr, "[QRY] RRT fallback (start proxy): OK (%d pts)\n", (int)path.size());
                    }
                    ...
                    if (!mid.empty()) {
                        ...
                        path.back() = goal;
                        goal_link_path.clear();
                        fprintf(stderr, "[QRY] RRT fallback (goal proxy): OK (%d pts)\n", (int)path.size());
                    }
                } else {
                    auto direct = rrt_connect(start, goal, checker, robot_, rrt_fb);
                    if (!direct.empty()) {
                        path = std::move(direct);
                        fprintf(stderr, "[QRY] RRT direct fallback: OK (%d pts)\n", (int)path.size());
                    }
                }
                ...
                if (path.empty()) {
                    ...
                    if (!direct.empty()) {
                        ...
                        start_link_path.clear();
                        goal_link_path.clear();
                        fprintf(stderr, "[QRY] RRT direct (last resort): OK (%d pts)\n", (int)path.size());
                    } else {
                        fprintf(stderr, "[QRY] all fallbacks FAIL\n");
                        return result;
                    }
                }
            }
    ...
    // ── Path composition diagnostics ──
    {
        ...
        fprintf(stderr, "[QRY] raw path: %d pts, len=%.3f  "
                "(start_link=%d, box_chain=%d boxes, goal_link=%d)\n",
                raw_pts, raw_len, n_start_link, n_box_chain, n_goal_link);
        fflush(stderr);
    }
    ...
    std::unordered_map<int, int> corridor_id_to_idx;
    {
        ...
        fprintf(stderr, "[QRY] corridor: %d boxes (seq=%d + neighbors)\n",
                (int)corridor_boxes.size(), (int)box_seq.size());
    }
    ...
    if (use_obs && path.size() > 3) {
        ...
        };
        {
            ...
            if (simplify_valid && len_after < len_before) {
                fprintf(stderr, "[QRY] pre-compete simplify: %d -> %d pts, len %.3f -> %.3f\n",
                        (int)chain_backup.size(), (int)simplified.size(), len_before, len_after);
                path = std::move(simplified);
            } else {
                fprintf(stderr, "[QRY] pre-compete simplify: %d -> %d pts, REVERTED (config invalid or no improvement)\n",
                        (int)chain_backup.size(), (int)simplified.size());
        ...
        fprintf(stderr, "[QRY] RRT budget: chain=%.3f euclid=%.3f ratio=%.2f → timeout=%.0fms\n",
                chain_len, euclid, ratio, rrt_timeout);
        ...
        if (!best_direct.empty() && best_direct_len < chain_len) {
            fprintf(stderr, "[QRY] RRT compete: direct=%.3f < chain=%.3f → using direct (%d/3 ok)\n",
                    best_direct_len, chain_len, n_success);
            ...
            fprintf(stderr, "[QRY] RRT compete: direct=%.3f >= chain=%.3f → keeping chain (%d/3 ok)\n",
                    best_direct_len, chain_len, n_success);
        } else {
            fprintf(stderr, "[QRY] RRT compete: all FAIL (budget=%.0fms) → keeping chain\n",
                    rrt_timeout);
        }
    }
    ...
    if (use_obs && path.size() > 2) {
        ...
        fprintf(stderr, "[OPT] greedy simplify: %d -> %d pts, len %.3f -> %.3f\n",
                (int)path.size(), (int)simplified.size(), len_before, len_after);
        path = std::move(simplified);
    }
    ...
    if (use_obs && path.size() > 8) {
        ...
        fprintf(stderr, "[OPT] random shortcut: len %.3f -> %.3f\n",
    ...
    if (path.size() > 2) {
        ...
        if (dense.size() > path.size()) {
            fprintf(stderr, "[OPT] densify: %d -> %d pts%s\n",
                    (int)path.size(), (int)dense.size(),
                    skipped > 0 ? (std::string(" (") + std::to_string(skipped) + " collision pts skipped)").c_str() : "");
            path = std::move(dense);
        }
    }
    ...
    if (path.size() > 3 && use_obs) {
        ...
        fprintf(stderr, "[OPT] elastic band: %d moves (%d iters), len %.3f -> %.3f\n",
                total_moves, eb_iters_done, len_before, len_after);
        ...
        if (eb_reverts > 0) {
            fprintf(stderr, "[OPT] elastic band: %d waypoints reverted (per-segment)\n",
                    eb_reverts);
        }
        ...
        if (!eb_clean) {
            fprintf(stderr, "[OPT] elastic band: FULL REVERT (per-segment fix insufficient)\n");
    ...
    if (use_obs && path.size() > 6) {
        ...
        if (!valid) {
            ...
            fprintf(stderr, "[OPT] final shortcut: REVERTED (collision)\n");
        } else {
            double len_after = sbf::path_length(path);
            if (len_after < len_before - 1e-6)
                fprintf(stderr, "[OPT] final shortcut: len %.3f -> %.3f\n",
                        len_before, len_after);
        }
    }
    ...
    if (use_obs && path.size() >= 3 && eb_improvement_pct >= 0.05) {
        ...
        // Mini elastic band (30 iterations)
        {
            ...
            if (moves2 > 0)
                fprintf(stderr, "[OPT] pass2 EB: %d moves, len %.3f -> %.3f\n",
                        moves2, len_before2, len_after2);
        }
        ...
        // pass2 collision revert: if pass2 introduced collision, revert entirely
        {
            ...
            if (!pass2_ok) {
                fprintf(stderr, "[OPT] pass2: REVERTED (collision)\n");
    ...
    if (use_obs && path.size() > 1) {
        ...
        if (!final_ok) {
            ...
            fprintf(stderr, "[OPT] FINAL SAFETY NET: path still has collision, "
                    "attempting emergency RRT\n");
            ...
            if (!empath.empty()) {
                ...
                fprintf(stderr, "[OPT] emergency RRT: %d pts, len=%.3f\n",
                        (int)path.size(), sbf::path_length(path));
            }
        }
    }
    ...
    // chain path or direct RRT, both known-shorter).
    {
        ...
        if (final_len > pre_validate_len * 1.5 && pre_validate_len > 1e-6) {
            fprintf(stderr, "[OPT] path regression (%.3f > 1.5×%.3f) → reverting to pre-validate\n",
                    final_len, pre_validate_len);
    ...
    fprintf(stderr, "[QRY-T] find=%.1fms bridge=%.1fms island=%.1fms dij+ext=%.1fms pre_simp=%.1fms\n"
                    "        rrt_compete=%.1fms validate=%.1fms greedy=%.1fms shortcut=%.1fms\n"
                    "        densify=%.1fms eb=%.1fms final_sc=%.1fms pass2=%.1fms safety=%.1fms total=%.1fms\n",
            dt_findbox, dt_bridge, dt_island, dt_dijext, dt_presimp,
            dt_rrtcomp, dt_validate, dt_greedy, dt_shortcut,
            dt_densify, dt_eb, dt_finalsc, dt_pass2, dt_safety,
            ms_since(t0, t1));
