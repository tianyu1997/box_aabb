/**
 * exp8_grid_margin.cpp — 实验 8: Grid margin 精细碰撞检测性能对比
 *
 * 方法:
 *   Grid / AABB-only  : LinkIAABB_Grid + margin_threshold=0 (纯 AABB)
 *   Grid / margin=0.01: AABB pass + 小幅穿入时 grid 精细检测 (1cm)
 *   Grid / margin=0.02: 同上，2cm 阈值
 *   Grid / margin=0.05: 同上，5cm 阈值
 *
 * 指标: Build(s), Boxes, Query(ms), SR%
 * 需求: envelope_type = LinkIAABB_Grid，n_threads=5 (parallel)
 *
 * 用法:
 *   ./exp8_grid_margin [--seeds N] [--quick]
 */
...
struct BenchConfig {
    std::string label;
    float  margin;      // 0 = AABB-only, >0 = margin-based grid
    int    n_threads;
};
...
SBFPlannerConfig make_cfg(const BenchConfig& bc, uint64_t seed, int bridge_threads) {
    ...
    // Margin-based grid refinement
    cfg.grower.ffb_config.grid_margin_threshold = bc.margin;
...
int main(int argc, char** argv) {
    ...
    std::cout << "Robot: " << robot.name() << "  DOF=" << robot.n_joints() << "\n"
              ...
              << "Seeds=" << n_seeds << "  Envelope=LinkIAABB_Grid  Threads=5\n\n";

    std::vector<BenchConfig> configs = {
        { "Grid / AABB-only (baseline)", 0.00f, 5 },
        { "Grid / margin=0.01",          0.01f, 5 },
        { "Grid / margin=0.02",          0.02f, 5 },
        { "Grid / margin=0.05",          0.05f, 5 },
    };

    constexpr int W = 100;
    std::cout << std::string(W, '=') << "\n"
              << "  Grid Margin Refinement — Build & Query Comparison\n"
              << std::string(W, '=') << "\n\n"
    ...
    std::cout << "\n" << std::string(W, '=') << "\n"
              << "  margin=0: AABB-only (baseline)\n"
              << "  margin>0: AABB pass + grid fine-check on marginal hits\n"
              << std::string(W, '=') << "\n";
    return 0;
}
