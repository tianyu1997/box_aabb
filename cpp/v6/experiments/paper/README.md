...
## Active paper scripts

- `run_all.py`: section-order orchestrator for retained paper wrappers.
...
- `06_sbf_obstacle_rebuild.py`: Exp.6 obstacle invalidation and repair.
- `07_update_paper_results.py`: regenerate paper tables/macros.
- `09_generate_paper_figures.py`: regenerate paper figures.

The current TRO manuscript source is `doc/paper/SBF/main.tex`. Generated
tables and macros are written to `doc/paper/SBF/generated/`.

## Shared implementation layer

- `common.py`: path conventions, build discovery, Python extension checks, and result writing.
- `exp5_scene_utils.py`: Exp.5 DH FK, AABB validation, workspace sampling, and scene constraints.
- `exp5_baselines.py`: reproducible baseline helpers for Exp.5 random scenes.
- `marcucci_envelope_build_replay.py`: retained Exp.3 implementation detail, not a standalone paper row.
- `../../scripts/run_online_query_comparison.py`: authoritative Exp.4 SBF build/query implementation.
- `results_frozen/`: small frozen references that are intended to stay versioned.

## Legacy or diagnostic scripts

These remain in place for archaeology and diagnostics, but are not paper-facing
unless explicitly requested: `02_6_link_envelope_growth_calibration.py`,
`03_1_marcucci_envelope_budget_scan.py`, `06_sbf_parameter_scan.py`,
`08_build_ablation_sweep.py`, `10_verify_exp5_prm_collision.py`,
`11_verify_exp5_prm_pybullet.py`, and `lect_ab_compare.py`.
...
- Run one heavy experiment at a time on a workstation.
- IRIS-NP and BIT* should be run as separate commands for full reruns.
- LECT caches are preserved during paper reruns unless a script explicitly resets them.

## Metrics fixed for Exp.3-6
...
- Exp.5 records SBF build and query separately after an untimed per-scene LECT
  ...
  URDF scenes, not the Marcucci OMPL/IRIS baselines.
- Exp.6 measures obstacle-change invalidation and graph repair; it should not be
  reported as a full rebuild unless the configuration says so explicitly.

## Usage principle

For paper reruns, enter through `run_all.py` or the numbered wrappers here. If a
script is not listed in `run_all.py`, treat it as diagnostic until documented
otherwise.
