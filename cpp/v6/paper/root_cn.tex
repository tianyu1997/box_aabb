...
与两种采样规划基线（RRT-Connect 和 3000 节点 PRM）
以及凸分解流水线 IRIS-NP + GCS~\cite{marcucci2024motion} 对比，
测试场景为 IIWA14 组合场景
（货架 + 料箱 + 桌面，16 个障碍物，5 组查询对，每组 3 个种子）。
GCS 使用 LinearGCS 求解器，L2 范数边代价，
凸松弛 + \texttt{randomForwardPathSearch} 路径舍入
（10 条路径、100 次试验），与原始实现一致。

\input{generated/tab4_baselines}

表~\ref{tab:baselines} 揭示三个发现：
\begin{enumerate}
  \item \textbf{SBF 预计算比 IRIS-NP 快 $54$ 倍。}
    SBF 在 2.4\,s 内构建 4650 个认证盒，
    而 IRIS-NP 生成 8 个区域需 130\,s。
    两者在全部 5 组查询对上均达 100\% 成功率，
    说明精心选取的种子可提供充分覆盖。
    IRIS-NP 路径略短（2.44 vs.\ 3.10\,rad），
    因 GCS 凸优化器直接最小化 L2 代价，
    而 SBF 在离散图上最小化跳数。

  \item \textbf{SBF 匹配 RRT-Connect 查询速度且路径更短。}
    中位查询时间几乎相同（0.164\,s vs.\ 0.160\,s），
    但 SBF 路径短 28\%（3.10 vs.\ 4.32\,rad），
    得益于密集盒图上 Dijkstra 搜索最小化关节空间代价。

  \item \textbf{PRM 在 7 自由度中受限于样本密度。}
    3000 个随机节点下，PRM 构建约 14\,000 条边的路线图
    耗时 7.8\,s，查询速度达亚毫秒级，
    但成功率仅 60\%：
    货架附近窄通道（TS$\to$CS）缺乏足够的
    无碰撞样本以形成连通路径。
\end{enumerate}
