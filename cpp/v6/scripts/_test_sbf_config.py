...
def make_combined_obstacles():
    def make_shelves():
        ...
        def add(lx, ly, lz, fx, fy, fz):
            obs.append(sbf5.Obstacle(ox+lx-fx/2, oy+ly-fy/2, oz+lz-fz/2,
    ...
    def make_bins():
        ...
        def add_bin(bx, by, bz):
            def add(lx, ly, lz, fx, fy, fz):
                obs.append(sbf5.Obstacle(bx-ly-fy/2, by+lx-fx/2, bz+lz-fz/2,
        ...
        return obs
    def make_table():
        return [sbf5.Obstacle(0.4-2.5/2, -2.5/2, -0.25-0.2/2, 0.4+2.5/2, 2.5/2, -0.25+0.2/2)]
...
robot = sbf5.Robot.from_json(os.path.join(os.path.dirname(__file__), '..', 'data', 'iiwa14.json'))
