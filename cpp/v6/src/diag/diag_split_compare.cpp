// SafeBoxForest v6 — Diagnostic: SplitOrder comparison
//
    ...
    std::vector<Row>& results)
{
    ...
    for (int hw_i = 0; hw_i < n_hw; ++hw_i) {
        ...
        for (int sc = 0; sc < N_SCENES; ++sc) {
            ...
            SBF_INFO(" [%s] hw=%.3f scene=%d done", robot_name, hw, sc);
        }
    }
}
...
int main() {
    SBF_INFO("SafeBoxForest v6 — SplitOrder Comparison (BT vs RR)");
    SBF_INFO("N_SEEDS=%d N_SCENES=%d max_depth=%d\n", N_SEEDS, N_SCENES, FFB_MAX_DEPTH);
    ...
    for (const char* rname : {"2dof", "panda"}) {
        for (const char* so : {"ROUND_ROBIN", "BEST_TIGHTEN"}) {
            ...
            if (cnt == 0) continue;
            SBF_INFO("[%s][%s] n=%d succ=%d avg_time=%.1fus avg_fk=%.1f avg_vol=%.6g", rname, so, cnt, succ, time_sum / cnt, double(fk_sum) / cnt, succ > 0 ? vol_sum / succ : 0.0);
        }
    }
