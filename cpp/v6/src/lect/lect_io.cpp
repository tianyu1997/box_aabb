...
static bool index_grid_section(const uint8_t* data, size_t len, LECT& lect) {
    ...
    SBF_INFO("[PLN] index_grid_section: %d entries skipped (no lazy)", n_entries);
    return true;
}
...
static bool load_v5_mmap(LECT& lect, const Robot& robot,
    ...
    // such dangling pointers to make the node a leaf again.
    {
        ...
        if (n_repaired > 0) {
            SBF_INFO("[PLN] lect_load: repaired %d nodes with dangling " "child pointers (>= n_nodes=%d)", n_repaired, nn);
        }
    }
...
bool lect_save_v6(const LECT& lect, const std::string& path) {
    ...
    SBF_INFO("[PLN] lect_save_v6: %d nodes, cap=%d, file≈%.1f KB", nn, cap, static_cast<double>(out.tellp()) / 1024.0);
...
bool lect_load_v6(LECT& lect, const Robot& robot, const std::string& path) {
    ...
    SBF_INFO("[PLN] lect_load_v6: %d nodes, cap=%d, %.1fms (tree-only, no EP)", nn, cap, ms);
