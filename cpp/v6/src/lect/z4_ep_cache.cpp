...
bool Z4EpCache::open(const std::string& path, int ep_stride,
    ...
    if (exists) {
        ...
        if (fd_ < 0) {
            SBF_WARN("[Z4EpCache] open(%s) failed: %s", path.c_str(), strerror(errno));
            return false;
        }

        Header hdr{};
        if (::pread(fd_, &hdr, sizeof(hdr), 0) != sizeof(hdr)) {
            SBF_WARN("[Z4EpCache] read header failed");
            ::close(fd_); fd_ = -1;
            return false;
        }
    ...
    if (!exists) {
        ...
        if (fd_ < 0) {
            SBF_WARN("[Z4EpCache] create(%s) failed: %s", path.c_str(), strerror(errno));
            return false;
        }
        if (::ftruncate(fd_, static_cast<off_t>(file_size_)) != 0) {
            SBF_WARN("[Z4EpCache] ftruncate failed: %s", strerror(errno));
            ::close(fd_); fd_ = -1;
            return false;
        }
        ...
        data_ = static_cast<uint8_t*>(
            ::mmap(nullptr, file_size_, PROT_READ | PROT_WRITE,
                   MAP_SHARED, fd_, 0));
        if (data_ == MAP_FAILED) {
            SBF_WARN("[Z4EpCache] mmap failed: %s", strerror(errno));
...
void Z4EpCache::grow() {
    ...
    if (new_fd < 0) {
        SBF_WARN("[Z4EpCache] grow: create tmp failed: %s", strerror(errno));
        return;
    }
    if (::ftruncate(new_fd, static_cast<off_t>(new_file_size)) != 0) {
        SBF_WARN("[Z4EpCache] grow: ftruncate failed");
        ::close(new_fd);
        return;
    }
    ...
    if (new_data == MAP_FAILED) {
        SBF_WARN("[Z4EpCache] grow: mmap failed");
        ::close(new_fd);
        return;
    }
