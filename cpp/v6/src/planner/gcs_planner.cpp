// SafeBoxForest v6 — GCS Planner (Phase H3 + Phase L)
    ...
    LECT* lect)
{
    ...
    SBF_INFO("[GCS] dijkstra path: %d boxes → shortcut: %d boxes", (int)dij.box_sequence.size(), (int)shortcut_seq.size());
    ...
    SBF_INFO("[GCS] corridor: %d unique → %d groups (max %d:1, %d collision splits)", (int)deduped_seq.size(), n_groups, group_size, n_collision_splits);
    ...
    SBF_INFO("[GCS] graph: %d verts, %d edges — solving...", (int)gcs.Vertices().size(), (int)gcs.Edges().size());
    // 6. Solve
    ...
    SBF_INFO("[GCS] solve done, success=%d, solver=%s, status=%s", (int)result.is_success(), result.get_solver_id().name().c_str(), result.get_solution_result() == drake::solvers::SolutionResult::kSolutionFound ? "found" : "other");
    ...
    SBF_INFO("[GCS] extracted: %d → %d waypoints (%d groups), cost=%.3f", (int)raw_pts.size(), (int)path.size(), n_groups, cost);
    ...
    if (checker != nullptr) {
        ...
        if (n_col_segs > 0) {
            SBF_INFO("[GCS] post-hoc: %d/%d segments in collision — " "returning raw waypoints (skip simplification)", n_col_segs, (int)(path.size() - 1));
            // Fall back to unsimplified path (finer waypoint resolution)
            return {true, std::move(raw_pts), cost, deduped_seq};
        }
    }
