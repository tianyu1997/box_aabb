// SafeBoxForest v6 — VizExporter implementation (Phase I, P)
...
static json serialize_intervals(const std::vector<Interval>& intervals)
{
    json ivs = json::array();
...
static json serialize_link_aabbs(const Robot& robot,
                                  const std::vector<Interval>& intervals)
{
    ...
    for (int a = 0; a < n_active; ++a) {
        ...
        lj["link_idx"] = alm[a];
        lj["aabb"] = {
            {aabbs[a * 6 + 0], aabbs[a * 6 + 3]},
            {aabbs[a * 6 + 1], aabbs[a * 6 + 4]},
            {aabbs[a * 6 + 2], aabbs[a * 6 + 5]}
        };
        links.push_back(lj);
    }
    return links;
...
static json serialize_obstacle(const Obstacle& o)
{
    ...
    return {{"center", {cx, cy, cz}}, {"half_sizes", {hx, hy, hz}}};
}
    ...
    const LECT& lect)
{
    ...
    for (const auto& box : boxes) {
        json bj;
        bj["box_id"] = box.id;
        bj["intervals"] = serialize_intervals(box.joint_intervals);
        bj["links"] = serialize_link_aabbs(robot, box.joint_intervals);
        jboxes.push_back(bj);
    }
    j["boxes"] = jboxes;
    ...
    const Obstacle* obs, int n_obs)
{
    ...
    for (int i = 0; i < n_obs; ++i)
        obstacles.push_back(serialize_obstacle(obs[i]));
    ...
    double voxel_delta)
{
    ...
    j["box_id"] = box.id;
    j["intervals"] = serialize_intervals(box.joint_intervals);
