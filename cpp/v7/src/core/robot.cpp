...
Robot Robot::from_json(const std::string& path) {
    ...
    if (j.contains("link_radii")) {
        for (auto& r : j["link_radii"]) radii.push_back(r.get<double>());
    }
    ...
    if (j.contains("coupled_pairs")) {
        for (auto& p : j["coupled_pairs"]) {
            robot.coupled_pairs_.emplace_back(p[0].get<int>(),
                                              p[1].get<int>());
        }
    }
    return robot;
...
uint64_t Robot::fingerprint() const {
    ...
    if (tool_frame_) {
        uint8_t has = 1; hash_bytes(&has, 1);
        ...
        uint8_t has = 0; hash_bytes(&has, 1);
    }

    // NOTE (D1): radii intentionally hashed here for v6 parity. P3 cache
    // key construction must skip radii bytes per architecture decision D1.
...
void Robot::pack_arrays() {
    ...
    if (n_total_links > 0) skip[0] = true;  // base link always skipped
    ...
    for (int i = 0; i < n_total_links; ++i)
        if (!skip[i]) active_link_map_.push_back(i);
