...
3. numerical/hybrid    - 混合策略（关键枚举 + 流形随机 + 随机补充）
...
from .strategies import CriticalStrategy, RandomStrategy, HybridStrategy
...
class AABBCalculator:
    ...
    def compute_envelope(
        ...
        Args:
            ...
            sampling: 'critical' | 'random' | 'hybrid'
            n_random_samples: random 模式每连杆采样数
            n_refine_samples: hybrid 模式每连杆随机补充数
        ...
        if method == 'numerical':
            ...
            if sampling == 'random':
                ...
                strategy_kwargs['n_refine_samples'] = n_refine_samples
                strategy_kwargs['critical_proximity_threshold'] = \
                    critical_proximity_threshold
    ...
    def compute_link_aabbs(
        ...
        """计算各连杆 AABB（旧兼容接口）"""
        result = self.compute_envelope(joint_intervals, method, sampling)
        return [AABBResult(a.min_point, a.max_point, method)
                for a in result.link_aabbs]

    def compute_end_effector_aabb(
        self,
        joint_intervals: List[Tuple[float, float]],
        method: str = 'numerical',
        sampling: str = 'critical',
        ...
        """计算末端执行器 AABB（旧兼容接口）"""
        results = self.compute_link_aabbs(joint_intervals, method, sampling)
    ...
    def compute_robot_aabb(
        ...
        if not results:
            return None
        mn = [min(r.min_point[i] for r in results) for i in range(3)]
        mx = [max(r.max_point[i] for r in results) for i in range(3)]
        return AABBResult(mn, mx, method)
