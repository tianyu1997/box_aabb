...
| BoxNode | BoxNode | Forest 中的节点对象，包含区间、体积、seed、邻接关系。含缓存 `center` 属性 |
| box forest | Box forest | 无重叠 box 集合及其邻接图 |
| 邻接 | Adjacency | 两个 box 在容差意义下共享边界/面，可用于拓扑连通 |
| strict 邻接 | Strict face-touching | 某维度精确接触且其余维度有重叠（`deoverlap.compute_adjacency`） |
| loose 邻接 | Loose overlap | 所有维度均有重叠（带容差），用于 bridge/Dijkstra（`_build_adjacency_and_islands`） |
...
| coarsen | Coarsen / 粗化 | 维度扫描合并相邻 box 以减少图节点数量 |
| bridge | Bridge / 桥接 | 在孤立连通分量间扩展桥接 box 或添加桥接边以连通 |
| island | Island / 孤岛 | box 图中的一个连通分量 |
| dry-run FFB | Dry-run find_free_box | `mark_occupied=False` 的试探性 FFB，不修改树状态 |
| boundary expansion | Boundary expansion | 在已有 box 外表面附近采样新 seed，改善局部连通性 |
| period | Period / 周期 | 关节空间的周期包裹（如旋转关节的 $[-\pi, \pi]$） |
| spatial index | Spatial index | 空间哈希索引，用于加速障碍物候选筛选 |
...
| GCS | Graph of Convex Sets | 可选路径优化分支（Drake） |
...
| UF | UnionFind | 并查集，用于连通分量检测 |
| BFS | Breadth-First Search | 广度优先搜索，用于可达性分析 |
| SOCP | Second-Order Cone Program | waypoint 优化中的凸优化形式 |
...
| 只读查询规划 | `planner.box_query.BoxForestQuery.plan` |
| seed 采样 | `BoxPlanner._sample_seed` |
| 边界扩展采样 | `BoxPlanner._sample_boundary_seed` |
| 层级扩展 | `forest.hier_aabb_tree.HierAABBTree.find_free_box` |
| 邻接构建（strict） | `forest.deoverlap.compute_adjacency` |
| 邻接构建（增量） | `forest.deoverlap.compute_adjacency_incremental` |
...
| box-aware 路径平滑 | `planner.path_smoother.PathSmoother.smooth_in_boxes` |
| box-aware shortcut | `planner.path_smoother.PathSmoother.shortcut_in_boxes` |
...
| 连通分量检测 | `forest.connectivity.find_islands` |
| 并查集 | `forest.connectivity.UnionFind` |
| 批量碰撞检测 | `forest.collision.CollisionChecker.check_config_collision_batch` |
...
| 分区合并 | `forest.box_forest.BoxForest.merge_partition_forests` |
| 不变量校验 | `forest.box_forest.BoxForest.validate_invariants` |
| 空间索引 | `forest.parallel_collision.SpatialIndex` |
| 路径指标评估 | `planner.metrics.evaluate_result` |
| 报告生成 | `planner.report.PlannerReportGenerator.generate` |
| 机械臂动画 | `planner.dynamic_visualizer.animate_robot_path` |
| 交互式查看器 | `planner.interactive_viewer.launch_viewer` |
