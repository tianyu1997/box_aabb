...
### D.2.1 子空间边界归属规则（避免重复）

- 对落在切分超平面上的 seed，采用确定性归属：
	- 默认归到“右子空间”（`q[dim] >= split_val`）；
	- 或配置 `boundary_owner = left/right`，全局唯一。
- 合并阶段若出现边界重叠碎片，按固定优先级（子空间 ID 小优先）去重。
...
worker-expand(worker_id, subspace_Qi, planner_cfg):
	...
	while not reach_budget:
		...
		if seed on boundary:
			seed = apply_boundary_owner_rule(seed)
...
9. **文件名**：`v2/src/planner/box_rrt.py`  
   **函数名**：`_plan_impl`  
   **变更点**：接入多进程分支：
   - 并行启动 worker；
   - 收集局部结果；
...
13. **文件名**：`v2/src/planner/box_rrt.py`  
...
15. **文件名**：`v2/src/planner/box_rrt.py`  
	**函数名**：`_plan_impl`、旧扩展分支函数  
	**变更点**：删除非 KD-tree 扩展模式入口与条件分支，仅保留 KD-tree 子空间并行主路径。

16. **文件名**：`v2/src/forest/box_forest.py`  
	**函数名**：`add_boxes` / `add_boxes_incremental`（视最终主路径）  
	**变更点**：清理与旧“全局切分 + 去重叠安全网”重复的路径，保留主路径必要逻辑。

17. **文件名**：`v2/src/forest/deoverlap.py`  
	**函数名**：`deoverlap`、`subtract_box`、相关调用方  
	**变更点**：若确认主路径不再依赖旧重叠处理，删除调用链并将模块降级为仅测试/工具（或移除）。
...
- 在 `PlannerConfig` 增加：
	...
	- `parallel_boundary_owner: str = "right"`（边界 seed 归属）
	- `parallel_cross_partition_connect: bool = True`
- 默认关闭，作为实验特性逐步放量。
...
- 代码改造清单（计划项）：
	...
	3. 删除旧重叠检测/去重叠路径（若与“子空间互斥 + 边界去重”重复）；
