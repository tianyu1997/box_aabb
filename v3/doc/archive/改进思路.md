...
1. **BoxForestQuery** (`box_query.py`)：轻量只读查询规划器，给定现有 forest 直接查路径
2. **metrics** (`metrics.py`)：`PathMetrics`, `evaluate_result`, `compare_results`, `format_comparison_table`
3. **interactive_viewer** (`interactive_viewer.py`)：TkAgg 交互式 2D 查看器，支持键盘控制
4. **dynamic_visualizer** (`dynamic_visualizer.py`)：`animate_robot_path` 动画生成
5. **compare_all_planners** (`examples/compare_all_planners.py`)：BoxPlanner 4 种并行配置 vs OMPL 5 种算法（RRT, RRTConnect, RRT\*, InformedRRT\*, BIT\*）统一 Panda 7-DOF 对比
6. **SpatialIndex** gating (`collision.py`)：碰撞检测自动启用空间索引（`spatial_index_threshold=20`）
7. **periodic bridge** (`connectivity.py`)：`bridge_islands` 支持 `period` 参数处理周期关节
