...
class SBFPlanner:
    ...
    def _generate_boundary_seeds(
        ...
        *,
        ...
        """为 box 的有效边界面生成 seed（带方向性的边缘采样）.

        Args:
            box: 源 box
            rng: 随机数生成器
            excluded_faces: 被排除的 (dim, side) 集合
            goal_point: 目标方向点；若提供则做 goal-biased face 选择
            n_samples: 每个 box 采样的面数 (0 = 所有有效面，即原始波前)
            active_dims: 限制扩张的维度列表 (None = 所有维度)

        Returns:
            [(dim, side, seed), ...] — 每个选中面返回一个 seed
        """
        eps = self.config.boundary_expand_epsilon
        n_dims = self._n_dims
        period = self._period
        dims_to_use = active_dims if active_dims is not None else list(range(n_dims))

        # ---- 1. 枚举有效面（排除 excluded_faces，仅 active_dims） ----
        ...
        # ---- 2. 面选择策略 ----
        if n_samples <= 0 or n_samples >= len(faces):
            # 全部面（原始波前行为 / 后连接随机扩张）
            rng.shuffle(faces)
            selected = faces
        elif goal_point is not None:
            # goal-biased: 计算 box 中心到 goal 的方向
            box_center = np.array(
                [0.5 * (box.joint_intervals[d][0] + box.joint_intervals[d][1])
                 for d in range(n_dims)],
                dtype=np.float64,
            )
            if period is not None:
                half = period / 2.0
                direction = ((goal_point - box_center) + half) % period - half
            else:
                direction = goal_point - box_center
            dir_norm = np.linalg.norm(direction)
            if dir_norm > 1e-12:
                direction /= dir_norm

            # 为每个面打分：法向量与 direction 的点积
            scored = []
            for d, s in faces:
                normal_sign = 2 * s - 1  # side=0 → -1, side=1 → +1
                score = normal_sign * direction[d]
                scored.append((score, d, s))
            scored.sort(key=lambda x: -x[0])  # 降序

            # 选最佳 1 个 goal-biased + (n_samples-1) 个随机
            best = (scored[0][1], scored[0][2])
            rest = [(d, s) for _, d, s in scored[1:]]
            rng.shuffle(rest)
            selected = [best] + rest[: n_samples - 1]
        else:
            # 纯随机（无 goal 信息时）
            rng.shuffle(faces)
            selected = faces[:n_samples]

        # ---- 3. 为选中面生成 seed ----
        results = []
        for dim, side in selected:
            ...
            # seed: expanding dim → boundary ± eps; 其余 dim → 面中心点
